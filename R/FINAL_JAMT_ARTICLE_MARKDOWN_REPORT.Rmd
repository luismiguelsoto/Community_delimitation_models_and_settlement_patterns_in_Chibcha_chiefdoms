---
title: Comparative analysis of settlement patterns and community delimitation models
  in Chibcha chiefdoms in the Intermediate Area.
...
cat("\n\n### Table 1 — Network Metrics Summary {#tab-network-metrics}\n\n")
all_network_summary <- do.call(rbind, network_summary_list)

print(
  tbl_kable(
    all_network_summary,
    caption = "Table 1. Network Metrics Summary",
...
if (length(all_curves) > 0) {
  ...
  # 
  cat("\n\n### Table 2 — Critical Connectivity Thresholds {#tab-critical-connectivity}\n\n")
  ...
  print(
    tbl_kable(
      table_data,
      caption = "Table 2. Critical Connectivity Thresholds",
...
for (region_name in names(all_data)) {
  ...
  net_comm_table <- rbind(net_comm_table, data.frame(
    ...
    hdbscan_k            = length(unique(na.omit(hdb_assign[hdb_assign > 0]))),
...
if (nrow(net_comm_table) > 0) {
  ...
  # 
  cat("\n\n### Table 3 — Method Convergence (ARI) and Network Detection {#tab-ari-convergence}\n\n")
  ...
  print(
    tbl_kable(
      ...
      caption = "Table 3. Method Convergence (Adjusted Rand Index) and Network Detection Summary",
...
for (region_name in names(all_data)) {
  ...
  mantel_test <- vegan::mantel(dist_geo, dist_area, method = "spearman", permutations = 100)
...
# 
cat("\n\n### Table 4 (Adapted): Mantel Test Results (Geo Distance vs Area Distance)\n\n")
...
```{r}
all_stats2 <- dplyr::bind_rows(lapply(names(all_data), function(region_name) {
  data <- all_data[[region_name]]
  trad <- traditional_results[[region_name]]
  hdb  <- hdbscan_results[[region_name]]
  net  <- network_results[[region_name]]
  ...
  #
  pts <- sf::st_as_sf(data$sites_df, coords = c("X", "Y"), crs = data$crs)
  D_nnd <- as.matrix(dist(sf::st_coordinates(pts))); diag(D_nnd) <- NA
  nnd_obs_vec   <- apply(D_nnd, 1, min, na.rm = TRUE)
  mean_nnd_obs  <- mean(nnd_obs_vec, na.rm = TRUE)
...
cat("### Master Summary Table of Regional Metrics\n\n")
print(knitr::kable(all_stats2 |> dplyr::mutate(across(where(is.numeric), ~ round(.x, 3))),
